...
namespace SmallCat.DynamicWebApi.Extensions;
...
public static class DynamicWebApiServiceExtensions
{
    ...
    public static IServiceCollection AddDynamicWebApi(this IServiceCollection services, DynamicWebApiOptions options)
    {
        ...
        if (partManager == null)
        {
            throw new InvalidOperationException($"\"{nameof(DynamicWebApiServiceExtensions.AddDynamicWebApi)}\" must be after \"AddMvc\".");
        }
        ...
        services.Configure<MvcOptions>(mvcOptions =>
        {
            ...
            mvcOptions.Conventions.Add(new DynamicWebApiConvention(options.SelectController, options.ActionRouteFactory));
        });
